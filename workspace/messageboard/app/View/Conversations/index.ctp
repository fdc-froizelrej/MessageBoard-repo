			...
			<div class="col-md-12">
				<div class="card mb-4" style="cursor: pointer;">
					<?php
						...
						if (!empty($conversation['Message']) && is_array($conversation['Message'])) {
							$lastMessage = $conversation['Message'][0];
							$lastMessageContent = $lastMessage['content'];
							if ($lastMessage['user_id'] == $currentUserId) {
								$lastMessageContent = __('You: ') . h($lastMessageContent);
							}
							$lastSentDate = h($conversation['last_sent_date']); 
						} else {
							$lastMessageContent = __('No messages yet');
							$lastSentDate = ''; 
						}
					?>
