    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-between">
                ...
                <div>
                    <?php echo $this->Html->link(__('Cancel'), ['action' => 'view', $user['User']['id']], ['class' => 'btn btn-outline-dark ml-2']);?>
                </div>
            </div>
            <?php echo $this->Form->create('User', array('class' => 'form-horizontal', 'enctype' => 'multipart/form-data')); ?>
                <div class="d-flex align-items-center">
                    <div class="border rounded p-2 me-2" style="width: 245px; height: 245px; overflow: hidden; display: flex; align-items: center; justify-content: center;">
                        <?php
                        // Set the current profile picture or a default one
                        $currentProfilePicture = !empty($user['User']['profile_picture']) 
                            ? h($this->Html->url('/uploads/profile_pictures/' . basename($user['User']['profile_picture']))) 
                            : 'https://static-00.iconduck.com/assets.00/profile-circle-icon-2048x2048-cqe5466q.png';
                        ?>
                        <img id="imageThumbnail" src="<?php echo $currentProfilePicture; ?>" alt="Image Preview" style="width: 100%; height: auto;"/>
                    </div>
                    <?php
                        echo $this->Form->input('profile_picture', array('type' => 'file', 'label' => false, 'style' => 'display: none;', 'accept' => 'image/jpeg, image/png, image/gif', 'id' => 'fileUpload'));
                    ?>
                    <button type="button" class="btn btn-outline-dark mx-3" onclick="document.getElementById('fileUpload').click();">
                        Upload Image
                    </button>
                </div>
                <fieldset>
                    <?php
                        ...
                        echo $this->Form->input('gender', array('type' => 'select', 'options' => array('Male' => 'Male', 'Female' => 'Female'), 'class' => 'form-control', 'label' => 'Gender'));
    ...
    $(function() {
        ...
        $("#fileUpload").change(function() {
            ...
            if (file) {
                ...
                $("#imageThumbnail").attr("src", "<?php echo h($this->Html->url('/uploads/profile_pictures/' . basename($user['User']['profile_picture']))); ?>"); // Reset to current picture if no file
            }
        });
    });
</script>
